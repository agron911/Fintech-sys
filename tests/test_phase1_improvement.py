...
def test_phase1_improvement_aapl(aapl_data, backtest_config_baseline, backtest_config_phase1,
    ...
    # If no trades generated by either, skip comparison (data may not produce signals)
    if baseline['results']['total_trades'] == 0 and phase1['results']['total_trades'] == 0:
        pytest.skip("No trades generated by baseline or Phase 1 — cannot measure improvement")
...
def test_phase1_no_regression_in_filtering(aapl_data, backtest_config_phase1, initial_capital):
    ...
    total_signals = phase1['filter_stats'].get('total_signals', 0)
    filter_pct = phase1['filter_stats'].get('filter_percentage', 0)

    # If no signals or no trades generated, skip (data-dependent benchmark)
    if total_signals == 0 or phase1['results']['total_trades'] == 0:
        pytest.skip(
            f"Insufficient signal generation for regression test "
            f"(signals={total_signals}, trades={phase1['results']['total_trades']})"
        )
